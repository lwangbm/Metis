...
class PolicyGradient:
    ...
    def learn(self, epoch_i, entropy_weight, IfPrint=False, alpha=0.5):
        discounted_ep_rs_norm_reward = self._discount_and_norm_rewards()
        discounted_ep_rs_norm_ss = self._discount_and_norm_rewards_ss()
        discounted_ep_rs_norm = alpha * discounted_ep_rs_norm_reward + (1-alpha) * discounted_ep_rs_norm_ss
        ...
        if IfPrint:
            print("epoch: %d, safety: %f,, tput: %f, entropy: %f, loss: %f" % (
                epoch_i, np.mean(self.ss_batch), np.mean(self.tput_batch), np.mean(entropy), np.mean(loss)))
